...
export default function Game({ player, players, date }) {
  const { getGameState, setGameState } = useGameContext();
  ...
  const makeGuess = (selectedPlayer) => {
    ...
    setGameState(date, { guesses: newGuesses, gameWon: newGameWon, gameLost: newGameLost, showImage: newShowImage });
    setCurrentGuess('');
    setSearchResults([]);
  };
...
ENDGAMEcat > /Users/kayrakocagil/Documents/mirsad-app/src/Game.jsx << 'ENDGAME'
import React, { useState, useEffect } from 'react';
import { useGameContext } from './GameContext';
